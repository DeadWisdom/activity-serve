...
async function handleRecaptchaFlow(authInstance, request, actionName, actionMethod, recaptchaAuthProvider) {
  if (recaptchaAuthProvider === "EMAIL_PASSWORD_PROVIDER") {
    if (authInstance._getRecaptchaConfig()?.isProviderEnabled("EMAIL_PASSWORD_PROVIDER")) {
    ...
    if (authInstance._getRecaptchaConfig()?.isProviderEnabled("PHONE_PROVIDER")) {
...
class IndexedDBLocalPersistence {
  ...
  async initializeSender() {
    ...
    if (results[0]?.fulfilled && results[0]?.value.includes("keyChanged")) {
...
async function signIn(providerName, extra) {
  await signInWithPopup(auth2, providers[providerName]);
...
globalThis.signOut = signOut2;

// node_modules/@lit/reactive-element/development/css-tag.js
var NODE_MODE = false;
var global2 = globalThis;
var supportsAdoptingStyleSheets = global2.ShadowRoot && (global2.ShadyCSS === undefined || global2.ShadyCSS.nativeShadow) && "adoptedStyleSheets" in Document.prototype && "replace" in CSSStyleSheet.prototype;
...
var getCompatibleStyle = supportsAdoptingStyleSheets || NODE_MODE && global2.CSSStyleSheet === undefined ? (s) => s : (s) => s instanceof CSSStyleSheet ? cssResultFromStyleSheet(s) : s;
...
if (DEV_MODE) {
  ...
  queueMicrotask(() => {
    ...
    if (global3.ShadyDOM?.inUse && polyfillSupport === undefined) {
...
global3.litPropertyMetadata ??= new WeakMap;
...
class ReactiveElement extends HTMLElement {
  ...
  static createProperty(name4, options = defaultPropertyDeclaration) {
    if (options.state) {
      options.attribute = false;
    }
    this.__prepare();
    if (this.prototype.hasOwnProperty(name4)) {
      options = Object.create(options);
      options.wrapped = true;
    }
    this.elementProperties.set(name4, options);
    if (!options.noAccessor) {
      const key = DEV_MODE ? Symbol.for(`${String(name4)} (@property() cache)`) : Symbol();
      const descriptor = this.getPropertyDescriptor(name4, key, options);
      if (descriptor !== undefined) {
        defineProperty(this.prototype, name4, descriptor);
      }
    }
  }
  static getPropertyDescriptor(name4, key, options) {
    const { get, set } = getOwnPropertyDescriptor(this.prototype, name4) ?? {
      get() {
        return this[key];
      },
      set(v) {
    ...
    if (DEV_MODE && get == null) {
      if ("value" in (getOwnPropertyDescriptor(this.prototype, name4) ?? {})) {
        throw new Error(`Field ${JSON.stringify(String(name4))} on ` + `${this.name} was declared as a reactive property ` + `but it's actually declared as a value on the prototype. ` + `Usually this is due to using @property or @state on a method.`);
      }
      issueWarning("reactive-property-without-getter", `Field ${JSON.stringify(String(name4))} on ` + `${this.name} was declared as a reactive property ` + `but it does not have a getter. This will be an error in a ` + `future version of Lit.`);
    }
    return {
      get,
      set(value) {
        ...
        set?.call(this, value);
        this.requestUpdate(name4, oldValue, options);
      },
      configurable: true,
      enumerable: true
    };
  }
  static getPropertyOptions(name4) {
    return this.elementProperties.get(name4) ?? defaultPropertyDeclaration;
  }
  static __prepare() {
  ...
  static __attributeNameForProperty(name4, options) {
    const attribute = options.attribute;
    return attribute === false ? undefined : typeof attribute === "string" ? attribute : typeof name4 === "string" ? name4.toLowerCase() : undefined;
  }
  constructor() {
    super();
  ...
  attributeChangedCallback(name4, _old, value) {
    this._$attributeToProperty(name4, value);
  }
  __propertyToAttribute(name4, value) {
    const elemProperties = this.constructor.elementProperties;
    const options = elemProperties.get(name4);
    const attr = this.constructor.__attributeNameForProperty(name4, options);
    if (attr !== undefined && options.reflect === true) {
      ...
      if (DEV_MODE && this.constructor.enabledWarnings.includes("migration") && attrValue === undefined) {
        issueWarning("undefined-attribute-value", `The attribute value for the ${name4} property is ` + `undefined on element ${this.localName}. The attribute will be ` + `removed, but in the previous version of \`ReactiveElement\`, ` + `the attribute would not have changed.`);
      }
      this.__reflectingProperty = name4;
      if (attrValue == null) {
  ...
  _$attributeToProperty(name4, value) {
    const ctor = this.constructor;
    const propName = ctor.__attributeToPropertyMap.get(name4);
  ...
  requestUpdate(name4, oldValue, options) {
    if (name4 !== undefined) {
      if (DEV_MODE && name4 instanceof Event) {
        issueWarning(``, `The requestUpdate() method was called with an Event as the property name. This is probably a mistake caused by binding this.requestUpdate as an event listener. Instead bind a function that will call it with no arguments: () => this.requestUpdate()`);
      }
      const ctor = this.constructor;
      const newValue = this[name4];
      options ??= ctor.getPropertyOptions(name4);
      const changed = (options.hasChanged ?? notEqual)(newValue, oldValue) || options.useDefault && options.reflect && newValue === this.__defaultValues?.get(name4) && !this.hasAttribute(ctor.__attributeNameForProperty(name4, options));
      if (changed) {
        this._$changeProperty(name4, oldValue, options);
      } else {
        return;
      }
    }
  ...
  _$changeProperty(name4, oldValue, { useDefault, reflect, wrapped }, initializeValue) {
    if (useDefault && !(this.__defaultValues ??= new Map).has(name4)) {
      this.__defaultValues.set(name4, initializeValue ?? oldValue ?? this[name4]);
      if (wrapped !== true || initializeValue !== undefined) {
        return;
      }
    }
    if (!this._$changedProperties.has(name4)) {
      if (!this.hasUpdated && !useDefault) {
        oldValue = undefined;
      }
      this._$changedProperties.set(name4, oldValue);
    }
    if (reflect === true && this.__reflectingProperty !== name4) {
      (this.__reflectingProperties ??= new Set).add(name4);
    }
  }
...
var wrap2 = ENABLE_SHADYDOM_NOPATCH && global4.ShadyDOM?.inUse && global4.ShadyDOM?.noPatch === true ? global4.ShadyDOM.wrap : (node) => node;
...
var createSanitizer = (node, name4, type) => {
  return sanitizerFactoryInternal(node, name4, type);
};
var boundAttributeSuffix = "$lit$";
var marker = `lit$${Math.random().toFixed(9).slice(2)}$`;
var markerMatch = "?" + marker;
var nodeMarker = `<${markerMatch}>`;
var d = NODE_MODE3 && global4.document === undefined ? {
  createTreeWalker() {
    return {};
  }
} : document;
    ...
    while ((node = walker.nextNode()) !== null && parts.length < partCount) {
      if (node.nodeType === 1) {
        ...
        if (node.hasAttributes()) {
          for (const name4 of node.getAttributeNames()) {
            if (name4.endsWith(boundAttributeSuffix)) {
              const realName = attrNames[attrNameIndex++];
              const value = node.getAttribute(name4);
            ...
            } else if (name4.startsWith(marker)) {
              parts.push({
                type: ELEMENT_PART,
                index: nodeIndex
              });
...
class AttributePart {
  ...
  constructor(element, name4, strings, parent, options) {
...
class BooleanAttributePart extends AttributePart {
  ...
  _commitValue(value) {
    ...
    wrap2(this.element).toggleAttribute(this.name, !!value && value !== nothing);
  }
}

class EventPart extends AttributePart {
  constructor(element, name4, strings, parent, options) {
    super(element, name4, strings, parent, options);
    this.type = EVENT_PART;
    if (DEV_MODE2 && this.strings !== undefined) {
      throw new Error(`A \`<${element.localName}>\` has a \`@${name4}=...\` listener with ` + "invalid content. Event listeners in templates must have exactly " + "one expression and no surrounding text.");
    }
  }
...
var issueWarning3;
if (DEV_MODE3) {
  global5.litIssuedWarnings ??= new Set;
...
var legacyProperty = (options, proto, name4) => {
  const hasOwnProperty = proto.hasOwnProperty(name4);
  proto.constructor.createProperty(name4, options);
  return hasOwnProperty ? Object.getOwnPropertyDescriptor(proto, name4) : undefined;
};
...
var standardProperty = (options = defaultPropertyDeclaration2, target, context) => {
  ...
  if (kind === "accessor") {
    ...
    return {
      ...
      init(v) {
        if (v !== undefined) {
          this._$changeProperty(name4, undefined, options, v);
        }
        return v;
      }
    };
...
// ui/components/auth-avatar.ts
class AuthAvatar extends LitElement {
  constructor() {
    super(...arguments);
  ...
  render() {
    ...
    if (this.user) {
      ...
      let name4 = this.user.displayName || "User";
      localStorage.setItem("photoUrl", this.user.photoURL || "");
      return html`
        <wa-dropdown>
          <wa-avatar slot="trigger" name=${name4} image=${photoURL}></wa-avatar>
          <wa-menu @wa-select=${this.handleSelect}>
            <wa-menu-label>${name4}<br/>${this.user.email}</wa-menu-label>
...
__legacyDecorateClassTS([
  state()
], AuthAvatar.prototype, "initialized", undefined);
__legacyDecorateClassTS([
  state()
], AuthAvatar.prototype, "user", undefined);
AuthAvatar = __legacyDecorateClassTS([
...
async function postToOutbox(activity) {
  let me = await getMe();
...
// ui/components/collection-feed.ts
class CollectionFeed extends LitElement {
  constructor() {
    super(...arguments);
    this.initialized = false;
    this.user = null;
  ...
  render() {
    ...
    return html`
      ...
      ${this.collection.items.map((item) => this.renderItem(item))}
    `;
  }
}
__legacyDecorateClassTS([
  state()
], CollectionFeed.prototype, "initialized", undefined);
__legacyDecorateClassTS([
  state()
], CollectionFeed.prototype, "user", undefined);
__legacyDecorateClassTS([
  state()
], CollectionFeed.prototype, "collection", undefined);
CollectionFeed = __legacyDecorateClassTS([
